...
public class CustomView: NSObject, UIGestureRecognizerDelegate {
    
    public var mBGBlack: UIView = {
        let mView = UIView()
    ...
    public var mContentView: UIView = {
        let mView = UIView()
    ...
    public var mCustomTable: CustomTableController!
    public var fHeightTop: CGFloat!
    public var bCustomViewIsShowing: Bool!
    
    public var ePosition: showPosition = .none
...
//            self.mCustomTable.bottomAnchor.constraint(equalTo: self.mContentView.bottomAnchor).isActive = true
    ...
    public func displayMenu(bIsShowMid: Bool = true) {
    ...
    @objc public func closeCustomView() {
        if self.bCustomViewIsShowing {
            UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.66, initialSpringVelocity: 1.0, options: .curveEaseOut, animations: {
    ...
    @objc public func handleScroll(_ recognizer: UIPanGestureRecognizer) {
